...
Presets live in the `progz.presets` namespace. Each is a frozen `Style`
instance; pass one as `style`, or use it as a base for
`dataclasses.replace`.

```python
from progz import ProgressBar, presets

with ProgressBar(total=100, style=presets.BLOCKS) as bar:
    ...
```

| Name                | Description                        |
|---------------------|-------------------------------------|
| `presets.SHIMMER` | Unique sine-wave brightness gradient, Unicode chars (default) |
| `presets.ASCII`   | `#`/`-` chars, `(BAR, DESCRIPTION)` layout (no spinner) |
| `presets.BLOCKS`  | Eighth-block sub-cell fill (`▏▎▍▌▋▊▉█`): 8 visible states per cell |
| `presets.MINIMAL` | Bar plus percent, nothing else     |
| `presets.DETAILED` | Every readout: spinner, bar, percent, count, rate, ETA, elapsed |
| `presets.RAINBOW` | Interpolated multi-color gradient painted across the bar |
